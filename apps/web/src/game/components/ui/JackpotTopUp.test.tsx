...
vi.mock('../../services/starknet', () => ({
  canCreateJackpot: vi.fn(),
  getJackpotPrizeAmount: vi.fn(),
}));
vi.mock('./WalletButton', () => ({
  WalletButton: () => <button>Connect wallet</button>,
}));

import { JackpotTopUp } from './JackpotTopUp';
...
function render(prize: Jackpot = jackpot, now = 99_999) {
  return renderToStaticMarkup(
    <JackpotTopUp jackpot={prize} now={now} onConfirmed={vi.fn()} />
  );
}

describe('jackpot top-up access', () => {
  beforeEach(() => {
    state.wallet = {
      address: '0xabc',
      chainId: '0x534e5f5345504f4c4941',
  ...
  it('closes at the deadline and during the draw', () => {
    expect(render(jackpot, 100_000)).toContain('Top-ups are closed');
    expect(render({ ...jackpot, status: 3 })).toContain('Top-ups are closed');
  });

  it('does not offer a top-up for an ERC-721 prize', () => {
    expect(render({ ...jackpot, prizeKind: 2, amount: 1n })).toBe('');
  });
});
