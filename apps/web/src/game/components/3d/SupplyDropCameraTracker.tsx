...
import {
  ...
  SUPPLY_DROP_RETURN_SECONDS,
  constrainSupplyDropCamera,
  createCameraPose,
  createSupplyDropCameraPath,
  interpolateCameraPose,
  supplyDropOrbitAngle,
  sampleSupplyDropCamera,
  type SupplyDropCameraPath,
} from '../../utils/supplyDropCamera';

export function SupplyDropCameraTracker({
  active,
  tracking,
  sectorId,
  onControlChange,
  ...
  const path = useRef<SupplyDropCameraPath | null>(null);
  ...
  useFrame((_state, delta) => {
    ...
    if (
      ...
      disposeControls();
      path.current = createSupplyDropCameraPath(sectorId, camera);
    ...
    if (controls.current) {
      ...
      if (!interacting.current && !prefersReducedMotion) {
        const angle =
          supplyDropOrbitAngle(idleElapsed.current - SUPPLY_DROP_HOLD_SECONDS) -
          supplyDropOrbitAngle(previousIdle - SUPPLY_DROP_HOLD_SECONDS);
        orbitOffset.current
      ...
      constrainSupplyDropCamera(camera.position, path.current.anchor);
    ...
    if (returning.current) {
      const progress = prefersReducedMotion
        ? 1
        : elapsed.current / SUPPLY_DROP_RETURN_SECONDS;
      interpolateCameraPose(
        returnStart.current,
      ...
      sampleSupplyDropCamera(
        path.current,
        prefersReducedMotion ? SUPPLY_DROP_FOCUS_SECONDS : elapsed.current,
        sample.current
      );
    }
    ...
    if (
      path.current &&
      !returning.current &&
      (prefersReducedMotion || elapsed.current >= SUPPLY_DROP_FOCUS_SECONDS)
    ) {
      ...
      orbitControls.minDistance = SUPPLY_DROP_MIN_DISTANCE;
      orbitControls.maxDistance = SUPPLY_DROP_MAX_DISTANCE;
      ...
      orbitControls.addEventListener('change', () => {
        constrainSupplyDropCamera(camera.position, anchor);
