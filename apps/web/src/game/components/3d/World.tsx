...
import { MAX_CONTROL_ACTION_BATCH } from '../../services/smartCapture';
  ...
  const finishMarquee = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (distance >= MARQUEE_DRAG_THRESHOLD_PX) {
        ...
        if (selected && selected.length > MAX_CONTROL_ACTION_BATCH) {
          notifyWarning(
            `That bounding box contains ${selected.length} Control Points. Select a smaller area with no more than ${MAX_CONTROL_ACTION_BATCH} points.`,
            'SELECTION LIMIT'
          );
        } else if (selected) {
