...
export function OrbitalBeacon({
  ...
  const orbitGeometry = useMemo(() => {
    const points = Array.from({ length: 161 }, (_, index) => {
      ...
      positionOnBeaconOrbit((index / 160) * Math.PI * 2, point);
      return point;
    });
  ...
  const beaconFaceFrameInverse = useMemo(() => {
    const positions = beaconGeometry.getAttribute('position');
    const normals = beaconGeometry.getAttribute('normal');
  ...
  useEffect(
    ...
    [beaconEdges, beaconGeometry, orbitGeometry]
  );
  ...
  useFrame(({ clock }, delta) => {
    ...
    const angle = beaconOrbitAngle(elapsedTime, prefersReducedMotion);

    if (orbitSystemRef.current) {
      orbitSystemRef.current.rotation.y = beaconOrbitPrecession(
        elapsedTime,
    ...
    if (beaconRef.current) {
      positionOnBeaconOrbit(angle, orbitPosition);
    ...
    if (bodyRef.current) {
      if (isProjectionSequenceActive) {
        ...
        tangentOnBeaconOrbit(angle, bodyProjectionTangent);
        bodyProjectionUp
        ...
        bodyProjectionTarget
          ...
          .multiply(beaconFaceFrameInverse);
  ...
  return (
    <group ref={orbitSystemRef}>
      ...
      <group ref={beaconRef}>
        ...
        >
          <mesh geometry={beaconGeometry} raycast={() => undefined}>
            <meshBasicMaterial
          ...
          <lineSegments geometry={beaconEdges} raycast={() => undefined}>
            <lineBasicMaterial
...
function BeaconProjection({
  ...
  useFrame(({ clock }) => {
    ...
    const angle = beaconOrbitAngle(
      clock.getElapsedTime(),
      prefersReducedMotion
    );
    positionOnBeaconOrbit(angle, projectionPosition);
