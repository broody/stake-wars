...
import {
  createBeaconBidStore,
  type BeaconBidPersistence,
  type StoredBeaconBid,
} from './beaconBidStorage';

class MemoryPersistence implements BeaconBidPersistence {
  readonly keys = new Map<string, CryptoKey>();
  readonly bids = new Map<
    string,
    Parameters<BeaconBidPersistence['putBid']>[0]
  >();
  ...
  async putBid(envelope: Parameters<BeaconBidPersistence['putBid']>[0]) {
...
describe('encrypted Beacon bid storage', () => {
  it('decrypts saved bids after a simulated reload and normalizes felts', async () => {
    const persistence = new MemoryPersistence();
    const crypto = webcrypto as unknown as Crypto;
    const firstPage = createBeaconBidStore(persistence, crypto);
    await firstPage.save(record);

    const reloadedPage = createBeaconBidStore(persistence, crypto);
    await expect(
      reloadedPage.list({
  ...
  it('does not expose the sealed amount or bid handle in its envelope', async () => {
    const persistence = new MemoryPersistence();
    const store = createBeaconBidStore(
      persistence,
