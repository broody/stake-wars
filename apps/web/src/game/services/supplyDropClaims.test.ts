...
describe('Supply Drop claim and staking', () => {
  ...
  it('allows direct claims for assets with an explicit non-staking policy', async () => {
    result('get_supply_drop_policy', ['0x7', '0x999', '0x0']);
    expect(
  ...
  it('rejects mismatched winner, already-claimed prize, and wrong token', async () => {
    ...
    claimed[20] = '0x1';
    result('get_supply_drop', claimed);
    await expect(prepareSupplyDropClaim(7n, winner)).rejects.toThrow(
      'not claimable'
    );
    const wrongToken = [...canonicalDrop];
    wrongToken[4] = '0x124';
    result('get_supply_drop', wrongToken);
    await expect(prepareSupplyDropClaim(7n, winner)).rejects.toThrow(
...
describe('Supply Drop authoritative reads', () => {
  it('rejects a missing policy instead of allowing a cash claim', async () => {
    result('get_supply_drop_policy', ['0x7', '0x0', '0x0']);
    await expect(prepareSupplyDropClaim(7n, winner)).rejects.toThrow(
      'Missing Supply Drop staking pool'
    );
  });
  it('fails closed when required entrypoints are missing', async () => {
    rpcError('get_supply_drop_policy', 21);
    rpcError('get_supply_drop_hold', 21);
    await expect(getSupplyDropPolicy(7n)).rejects.toThrow();
    await expect(getSupplyDropHold(winner)).rejects.toThrow();
  });
  it.each([20, 40, -32603])(
