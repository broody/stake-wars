import type { PreparedBeaconImage } from '../services/api';

export const BEACON_DETAIL_MAX_DIMENSION = 512;
export const BEACON_THUMBNAIL_MAX_DIMENSION = 256;
...
export async function prepareBeaconImage(
  file: File,
  maximumEncodedBytes: number
): Promise<PreparedBeaconImage> {
  ...
  try {
    const detailCanvas = renderBillboard(
      ...
      bitmap.width,
      bitmap.height,
      BEACON_DETAIL_MAX_DIMENSION
    );
    const thumbnailCanvas = renderBillboard(
      bitmap,
      bitmap.width,
      bitmap.height,
      BEACON_THUMBNAIL_MAX_DIMENSION
    );
