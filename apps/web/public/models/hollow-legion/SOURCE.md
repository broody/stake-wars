...
glTF is Y-up and the Mite faces +Z. These are art asset exports; gameplay integration and animated swarm rendering have not been implemented.
...
- `lancer.glb` — 722 triangles, 17 bones, a two-second Idle loop, and a Muzzle attachment bone; 104,860 bytes.
- `lancer-instanced.glb` — the same geometry in a static pose, without skin or animation; 57,428 bytes.

Lancer is approximately 1.918 m tall and faces +Z in glTF. The integrated cannon replaces its right hand. Walking, firing animations, projectile logic, and gameplay integration are not implemented in this first pass.
