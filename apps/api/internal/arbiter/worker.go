...
// Duty is one idempotent piece of periodic Arbiter maintenance.
type Duty interface {
...
func (d *AuctionCycleDuty) Reconcile(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("read current Arbiter round: %w", err)
	}
	...
	if err := d.restarter.EnsureNextRound(ctx, outcome); err != nil {
		return fmt.Errorf("ensure next Arbiter round: %w", err)
	}
	return nil
}
