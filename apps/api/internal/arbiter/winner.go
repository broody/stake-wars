...
func (p *WinnerProjector) Reconcile(ctx context.Context) error {
	...
	for _, winner := range winners {
		...
		if err != nil {
			return fmt.Errorf("resolve Arbiter winner for round %d: %w", winner.RoundID, err)
		}
		if disclosure.Status != "winner" {
			return fmt.Errorf(
				"resolve Arbiter winner for round %d: operator returned %q",
				winner.RoundID,
				disclosure.Status,
			)
		}
		...
		if err != nil || auctionID != winner.AuctionID {
			return fmt.Errorf("resolve Arbiter winner for round %d: auction ID mismatch", winner.RoundID)
		}
		winnerGroup, err := starknet.NormalizeFelt(disclosure.WinnerGroupHandle)
		if err != nil || winnerGroup != winner.WinnerGroupHandle {
			return fmt.Errorf("resolve Arbiter winner for round %d: winner group mismatch", winner.RoundID)
		}
		...
		if err != nil || winnerCommitment != winner.WinnerCommitment {
			return fmt.Errorf("resolve Arbiter winner for round %d: winner commitment mismatch", winner.RoundID)
		}
		address, err := starknet.NormalizeAddress(disclosure.Address)
		if err != nil {
			return fmt.Errorf("resolve Arbiter winner for round %d: invalid address: %w", winner.RoundID, err)
		}
