...
func (s *Service) Current(ctx context.Context) (Snapshot, error) {
	...
	if err != nil {
		return Snapshot{}, fmt.Errorf("validate canonical Beacon round: invalid Whisper address: %w", err)
	}
	...
	if auction.Status == starknet.WhisperStatusSettled {
		...
		if result.AuctionID != round.AuctionID {
			return Snapshot{}, fmt.Errorf(
				"validate canonical Beacon round: result auction id is %d, expected %d",
				result.AuctionID,
				round.AuctionID,
			)
		}
		if result.SettlementHash != auction.SettlementHash {
			return Snapshot{}, fmt.Errorf("validate canonical Beacon round: settlement hash mismatch")
		}
	...
	if err == nil {
		...
		if err != nil {
			return Snapshot{}, fmt.Errorf("validate claimed Beacon controller: %w", err)
		}
...
func validateCanonicalRound(
	...
	if auction.ID != round.AuctionID {
		return fmt.Errorf(
			"validate canonical Beacon round: auction id is %d, expected %d",
			auction.ID,
			round.AuctionID,
		)
	}
	if auction.Status == starknet.WhisperStatusPending {
		if auction.Schedule.Kind != starknet.WhisperScheduleStartOnBid ||
			...
			return fmt.Errorf("validate canonical Beacon round: invalid pending schedule")
		}
	} else if auction.BiddingDeadline >= auction.ForceRevealAfter ||
		auction.ForceRevealAfter >= auction.AbortAfter {
		return fmt.Errorf("validate canonical Beacon round: invalid auction deadlines")
	}
	if auction.Schedule.Kind == starknet.WhisperScheduleStartOnBid &&
		(auction.Schedule.BiddingDuration == 0 ||
			auction.Schedule.AcceptanceDuration == 0 ||
			auction.Schedule.SettlementDuration == 0) {
		return fmt.Errorf("validate canonical Beacon round: invalid start-on-bid durations")
	}
	if auction.Schedule.Kind == starknet.WhisperScheduleStartOnBid &&
		auction.Schedule.BiddingDuration != expectedBiddingDurationSeconds {
		return fmt.Errorf(
			"validate canonical Beacon round: bidding window is %d seconds, expected %d",
	...
	if auction.Schedule.Kind != starknet.WhisperScheduleAbsolute &&
		...
		return fmt.Errorf("validate canonical Beacon round: unsupported auction schedule")
	}
	if auction.Schedule.Kind == starknet.WhisperScheduleAbsolute &&
		...
		return fmt.Errorf("validate canonical Beacon round: absolute schedule mismatch")
	}
	if auction.Schedule.Kind == starknet.WhisperScheduleStartOnBid &&
		...
		return fmt.Errorf("validate canonical Beacon round: resolved schedule mismatch")
	}
	if auction.FulfillmentKind != starknet.WhisperFulfillmentOffchain ||
		...
		return fmt.Errorf("validate canonical Beacon round: fulfillment must be offchain")
	}
	checks := []struct {
		name     string
		expected string
	...
	for _, check := range checks {
		...
		if err != nil {
			return fmt.Errorf("validate canonical Beacon round: invalid %s: %w", check.name, err)
		}
		actual, err := normalize(check.actual)
		if err != nil {
			return fmt.Errorf("validate canonical Beacon round: invalid on-chain %s: %w", check.name, err)
		}
		if expected != actual {
			return fmt.Errorf("validate canonical Beacon round: %s mismatch", check.name)
		}
	}
	return nil
}
