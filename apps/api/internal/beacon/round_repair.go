...
func NewRoundLabelRepairer(
	...
	if store == nil || reader == nil {
		return nil, fmt.Errorf("Beacon round repair dependencies are required")
	}
	network = strings.TrimSpace(network)
	if network == "" {
		return nil, fmt.Errorf("Beacon round repair network is required")
	}
...
func (r *RoundLabelRepairer) Repair(ctx context.Context, spec RoundLabelRepair) error {
	if spec.ExistingRoundID == 0 || spec.ExistingAuctionID == 0 ||
		...
		return fmt.Errorf("Beacon round repair IDs must be positive")
	}
	if spec.CorrectedRoundID == spec.ExistingRoundID ||
		spec.ExistingAuctionID == spec.MissingAuctionID {
		return fmt.Errorf("Beacon round repair identities must be distinct")
	}
	...
	if err := validateCanonicalRound(
		...
		return fmt.Errorf("verify incorrectly labelled Beacon round: %w", err)
	}
	if err := validateCanonicalRound(
		missingRound, missingAuction, missingRound.BiddingDurationSeconds,
	); err != nil {
		return fmt.Errorf("verify missing Beacon round: %w", err)
	}
	if existingAuction.Status == starknet.WhisperStatusSettled ||
		existingAuction.Status == starknet.WhisperStatusAborted {
		return fmt.Errorf("incorrectly labelled Beacon round is already terminal")
	}
...
func (s *Store) relabelAndInsertRound(
	...
	if err != nil {
		return fmt.Errorf("begin Beacon round label repair: %w", err)
	}
	...
	if correctedErr == nil && missingErr == nil {
		if !sameCanonicalRoundIdentity(correctedStored, corrected) ||
			...
			return fmt.Errorf("repair Beacon round labels: conflicting repaired state")
		}
		if correctedStored.BiddingDurationSeconds != corrected.BiddingDurationSeconds ||
			...
			return fmt.Errorf("repair Beacon round labels: conflicting repaired schedule")
		}
		return tx.Commit()
	}
	if !errors.Is(correctedErr, ErrNoRound) || missingErr != nil {
		return fmt.Errorf("repair Beacon round labels: unexpected target state")
	}
	if !sameCanonicalRoundIdentity(missingStored, existing) {
		return fmt.Errorf("repair Beacon round labels: existing round identity mismatch")
	}

	for _, table := range []string{
		"beacon_round_outcomes", "beacon_image_uploads", "beacon_artworks",
	} {
		var count int
		query := fmt.Sprintf("SELECT COUNT(*) FROM %s WHERE network = ? AND %s = ?", table,
			map[string]string{
				"beacon_round_outcomes": "round_id",
				"beacon_image_uploads":  "controller_round_id",
				"beacon_artworks":       "controller_round_id",
			}[table],
		)
		if err := tx.QueryRowContext(ctx, query, existing.Network, existing.RoundID).Scan(&count); err != nil {
			return fmt.Errorf("inspect Beacon round repair dependencies: %w", err)
		}
		if count != 0 {
			return fmt.Errorf("repair Beacon round labels: existing round has dependent records")
		}
	}
	var predecessorJobs int
	if err := tx.QueryRowContext(ctx, `
		SELECT COUNT(*) FROM beacon_cycle_jobs
		WHERE network = ? AND predecessor_round_id = ?
	`, existing.Network, existing.RoundID).Scan(&predecessorJobs); err != nil {
		return fmt.Errorf("inspect Beacon cycle repair dependencies: %w", err)
	}
	if predecessorJobs != 0 {
		return fmt.Errorf("repair Beacon round labels: existing round has a successor job")
	}
	...
	if err != nil {
		return fmt.Errorf("relabel Beacon round: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil || rows != 1 {
		return fmt.Errorf("relabel Beacon round: round was not updated")
	}
	...
	if err != nil {
		return fmt.Errorf("insert missing Beacon round: %w", err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit Beacon round label repair: %w", err)
	}
	return nil
}
