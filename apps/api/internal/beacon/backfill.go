...
// be projected into Beacon history. Callers must enumerate rounds explicitly;
...
func newHistoricalRoundBackfiller(
	...
	if store == nil || reader == nil || source == nil {
		return nil, fmt.Errorf("historical Beacon backfill dependencies are required")
	}
	network = strings.TrimSpace(network)
	if network == "" {
		return nil, fmt.Errorf("historical Beacon backfill network is required")
	}
...
func (b *HistoricalRoundBackfiller) Backfill(
	...
	if spec.RoundID == 0 || spec.AuctionID == 0 {
		return fmt.Errorf("historical Beacon round and auction IDs must be positive")
	}
	...
	if err := validateCanonicalRound(round, auction, expectedBiddingDuration); err != nil {
		return fmt.Errorf("verify historical Beacon round %d: %w", spec.RoundID, err)
	}
	...
	if err := verifier.verifySettlement(ctx, round, projection); err != nil {
		return fmt.Errorf("verify historical Beacon settlement for round %d: %w", spec.RoundID, err)
	}
