...
func TestDutyLocksExpiredActiveSupplyDrop(t *testing.T) {
	reader := &fakeReader{
		supply_drop: starknet.SupplyDropState{ID: 7, Status: statusActive, EndsAt: 100},
...
func TestDutyWaitsUntilSupplyDropExpiry(t *testing.T) {
	reader := &fakeReader{
		supply_drop: starknet.SupplyDropState{ID: 7, Status: statusActive, EndsAt: 101},
		head:        starknet.ChainHead{BlockNumber: 200, Timestamp: 100},
	}
	submitter := &fakeSubmitter{}
...
func TestDutySettlesWhenRandomnessIsAvailable(t *testing.T) {
	reader := &fakeReader{
		supply_drop: starknet.SupplyDropState{
...
func TestDutyTreatsNoActiveSupplyDropAsIdle(t *testing.T) {
	reader := &fakeReader{err: starknet.ErrNoActiveSupplyDrop}
...
func (r *fakeReader) ActiveSupplyDrop(context.Context) (starknet.SupplyDropState, error) {
	return r.supply_drop, r.err
}
...
func (s *fakeSubmitter) LockSupplyDrop(_ context.Context, id uint64) (string, error) {
	s.locked = id
	return "0xlock", s.err
}

func (s *fakeSubmitter) SettleSupplyDrop(_ context.Context, id uint64) (string, error) {
	s.settled = id
