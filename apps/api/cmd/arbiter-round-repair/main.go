...
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "Arbiter round label repair failed")
		os.Exit(1)
	}
	fmt.Println("Arbiter round labels are repaired")
}
...
func run() error {
	...
	defer db.Close()
	repairer, err := arbiter.NewRoundLabelRepairer(
		arbiter.NewStore(db),
	...
	return repairer.Repair(ctx, arbiter.RoundLabelRepair{
