...
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "Beacon history backfill failed")
		os.Exit(1)
	}
	fmt.Println("Beacon history backfill is complete")
}
...
func run() error {
	...
	defer db.Close()
	backfiller, err := beacon.NewHistoricalRoundBackfiller(
		beacon.NewStore(db),
...
func parseRounds(value string) ([]beacon.HistoricalRound, error) {
	parts := strings.Split(strings.TrimSpace(value), ",")
	if len(parts) == 1 && strings.TrimSpace(parts[0]) == "" {
		return nil, fmt.Errorf("at least one round:auction pair is required")
	}
	rounds := make([]beacon.HistoricalRound, 0, len(parts))
	...
	for _, part := range parts {
		...
		rounds = append(rounds, beacon.HistoricalRound{RoundID: roundID, AuctionID: auctionID})
	}
	return rounds, nil
}
