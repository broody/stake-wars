...
pub mod control {
    ...
    impl ControlImpl of IControl<ContractState> {
        fn capture(ref self: ContractState, control_point_id: u32, allocation: u128) {
            ...
            let (operator, live_amount) = self.refresh_operator(caller, config.staking_pool);
            self
                .capture_with_synced(
        ...
        fn reinforce(ref self: ContractState, control_point_id: u32, additional_allocation: u128) {
            ...
            let caller = get_caller_address();
            let (mut operator, live_amount) = self.refresh_operator(caller, config.staking_pool);
        ...
        fn release(ref self: ContractState, control_point_id: u32) {
            ...
            let caller = get_caller_address();
            let (mut operator, _) = self.refresh_operator(caller, config.staking_pool);
            let mut world = self.world_default();
            let mut point: ControlPoint = world.read_model(control_point_id);
            assert(
                point.controller == caller && point.controller_generation == operator.generation,
        ...
        fn sync_operator(ref self: ContractState, operator: ContractAddress) -> u128 {
            ...
            let (_, live_amount) = self.refresh_operator(operator, config.staking_pool);
            live_amount
        }
        ...
        fn required_stake(self: @ContractState, control_point_id: u32) -> u128 {
            ...
            let operator: OperatorState = world.read_model(point.controller);
            let live_amount = delegated_amount(config.staking_pool, point.controller);
            if point.controller_generation != operator.generation
                || live_amount < operator.total_allocated {
                return config.minimum_stake;
            }
        ...
        fn available_stake(self: @ContractState, operator: ContractAddress) -> u128 {
            let config = self.initialized_config();
            let world = self.world_default();
            let state: OperatorState = world.read_model(operator);
            let live_amount = delegated_amount(config.staking_pool, operator);
            if live_amount < state.total_allocated {
                return live_amount;
            }
            live_amount - state.total_allocated
        }
    }
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn capture_with_synced(
            ...
            if !point.controller.is_zero() {
                ...
                if point.controller_generation == current_operator.generation {
